...
- 実績達成履歴はセッションを跨いで保持する想定。メタデータ保存は既存 saveStorage と同等の JSON 保存を追加し、初期読み込み/リセット API を用意する。
...
- 報酬種別ごとに処理を分岐
  ...
  - 記憶ポイント: 実績ストアの残高を加算。
- 再取得時は記憶ポイント残高を減算し、成功時のみ状態を「獲得済み」へ。
...
2. **記憶ポイントの扱い**
   - ラン限定でリセット。初期値は 0（あるいは指定初期値があれば教えてください）。
3. **再取得のコストとルール**
