...
- メインフェイズ
    - play-card１枚目：「被虐のオーラ」を「かたつむり」に使用する
        - 検証すべき結果
            ...
            - かたつむりが「溶かす」の行動をする
            - プレイヤーが攻撃を受けたことにより
                - プレイヤーの手札が6枚になり、状態異常「腐食」とアタック「溶かす」を追加される。使用された「被虐のオーラ」は捨て札に移動する。
...
- メインフェイズ
    ...
    - play-card2枚目：「溶かす」を「触手」に使用する
        - 検証すべき結果
            - マナが2になる
            - 触手に5ダメージ。触手のHPが20になり、Stateが「腐食(1)」になる
            - 手札の「溶かす」が捨て札に移動し、手札が8枚になる
...
- メインフェイズ
    - play-card１枚目：「疼き」を手札の「突き刺す 10×4」に使用する
        - 検証すべき結果
            - マナが2になる
            - 手札の「疼き」が消滅ゾーンに送られ、手札にもう一枚の「突き刺す」が追加される。手札に10×4ダメージの「突き刺す」が2枚ある。手札が9枚になっている。
    - play-card2枚目：「突き刺す 10×4」を「オーク」に使用
        - 検証すべき結果
            - マナが1になる
            - オークに40ダメージ。HPが0になり、オークが撃破状態になる。
    - play-card3枚目：「突き刺す 10×4」を「オークダンサー」に使用
        - 検証すべき結果
            - マナが0になる
