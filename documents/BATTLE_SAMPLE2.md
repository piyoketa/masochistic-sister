...
- メインフェイズ
    - play-card１枚目：「被虐のオーラ」を「鉄花」に使用する
        - 検証すべき結果
            ...
            - 鉄花が「粘液飛ばし」を即時発動する
            - プレイヤーのHPが145になる
            - 追加効果として手札に「粘液飛ばし(5)」と状態異常カード「粘液(1)」が加わり、手札6枚になる
    ...
    - play-card２枚目：「天の鎖」を「なめくじ」に使用する
        - 検証すべき結果
            ...
            - 手札は5枚になる（天の鎖・戦いの準備・日課・粘液飛ばし・粘液）
...
検証すべき結果
- プレイヤーのHPが120になる
- 手札が5枚で、内訳は「天の鎖」「日課」「粘液飛ばし(5)」「粘液」「乱れ突き(5×5)」
...
- メインフェイズ
    ...
    - play-card４枚目：「日課」を使用する
        - 検証すべき結果
            ...
            - 使用した「日課」が捨て札に移動する
            - この時点の手札は「天の鎖」×3、「粘液飛ばし(5)」、「粘液」、「再装填」である
...
検証すべき結果
- 手札が8枚で、内訳は「天の鎖」×3、「粘液飛ばし(5)」、「再装填」、「酸を吐く(5)」、「粘液」、「腐食」
- プレイヤーのHPが75
- 敵の行動（Action）名が全て上記通り
(ここまでがend-player-turn の処理)
